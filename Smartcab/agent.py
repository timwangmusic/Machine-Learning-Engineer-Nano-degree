...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        # self.epsilon = np.exp(-self.trials * self.decay_factor)
        if testing:
            self.epsilon = 0.0
            self.alpha = 0.0
        self.trials += 1
    ...
    def choose_action(self, state):
        ...
        if not self.learning or decision(self.epsilon):
            return random.choice(self.valid_actions)

        maxQ = self.get_maxQ(state)

        L = filter(lambda x: self.Q[state][x] == maxQ, self.valid_actions)
